    ...
    def get_frames_mouth(self, detector, predictor, frames):
        ...
        for frame in frames:
            ...
            for k, d in enumerate(dets):
                ...
                #for j in range(68):
                    #cv2.circle(frame, (shape.part(j).x, shape.part(j).y), 5, (0,255,0), -1)

                #cv2.imshow("suck.jpg", frame)
                #cv2.imwrite('ddd.jpg', frame)
                i = -1
            if shape is None:
                print('shape noenoe')
                # Detector doesn't detect face, just return as is
                return mouth_frames
            else:
                print('dadada')
            mouth_points = []
            ...
            print(mouth_crop_image.shape)
            print(mouth_l, mouth_r, mouth_t, mouth_b)
            mouth_frames.append(mouth_crop_image)
            # print(len(mouth_frames[0]))
        return mouth_frames
